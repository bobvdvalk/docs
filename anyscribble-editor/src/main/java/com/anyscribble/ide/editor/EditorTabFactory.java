...
public class EditorTabFactory {
    ...
    public Tab buildTab(Path file) {
        Path nomalizedPath = normalized(file);
        Tab tab = new Tab(nomalizedPath.getFileName().toString());
        ...
        if (controller != null) {
            controller.loadFile(nomalizedPath);
            controllerCache.put(nomalizedPath, new EditorTabData(tab, controller));
        }

        tab.setOnClosed(event -> controllerCache.remove(nomalizedPath));
